import React from 'react'

const OurStory = () => {
    return (

        <div className='our-story-container'>
            
            <h2 className='our-story-title'>המסע שלנו יחד איתך</h2>

            <div className='step-container'>
                <div className='step-right-side'>
                    <div className='step-card right num2'>
                        <span className='image-circle'>
                            <img src="img/compass.svg" alt="compass" />
                        </span>

                        <h3>פיתוח אתרים וחנויות בקוד פתוח (Full Stack)</h3>
                        <p>נאפיין במדויק את הצרכים והפונקציונליות של האתר, עם התאמה טכנית מושלמת. נתמקד בחוויית משתמש (UX) ובממשק משתמש (UI) שיעניקו למבקרים חוויה יוצאת דופן.</p>
                    </div>
                    <div className='step-card right num4'>
                        <span className='image-circle'>
                            <img src="img/rocket-02.svg" alt="" />
                        </span>
                        <h3>פיתוח אסטרטגיה שיווקית</h3>
                        <p>נלווה אותך בתהליך ההשקה ונוודא שהכל פועל חלק. גם לאחר מכן, אנחנו כאן בשבילך עם תמיכה ועדכונים כדי לשמור על האתר שלך תמיד צעד אחד קדימה.</p>
                    </div>
                </div>

                <div className='step-center'>
                    <img src="img/line.svg" alt="line" />
                    <p className='number num1'>1</p>
                    <p className='number num2'>2</p>
                    <p className='number num3'>3</p>
                    <p className='number num4'>4</p>
                </div>

                <div className='step-left-side'>
                    <div className='step-card left num1'>
                        <span className='image-circle'>
                            <img src="img/face-happy.svg" alt="compass" />
                        </span>

                        <h3>היכרות מעמיקה</h3>
                        <p>נתחיל בשיחה אישית כדי להבין את החזון, המטרות והערכים שלך. נבצע מחקר קהל יעד ומחקר שוק כדי לבנות אסטרטגיה מותאמת שתבליט אותך בשוק התחרותי.</p>
                    </div>
                    <div className='step-card left num3'>
                        <span className='image-circle'>
                            <img src="img/Frame 6481.svg" alt="" />
                        </span>
                        <h3>פיתוח אתרים וחנויות בקוד פתוח (Full Stack)</h3>
                        <p>נאפיין במדויק את הצרכים והפונקציונליות של האתר, עם התאמה טכנית מושלמת. נתמקד בחוויית משתמש (UX) ובממשק משתמש (UI) שיעניקו למבקרים חוויה יוצאת דופן.</p>
                    </div>
                </div>
            </div>

        </div>
    )
}

export default OurStory